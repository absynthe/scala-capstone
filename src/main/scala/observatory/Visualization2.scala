...
object Visualization2 {
  ...
  def pixelLocations(offsetX: Int, offsetY: Int, zoom: Int, imageWidth: Int, imageHeight: Int):IndexedSeq[(Int, Location)] = {
    ...
    } yield xPixel + yPixel * imageWidth -> tileLocation(
      (xPixel.toDouble / imageWidth + offsetX).toInt,
      (yPixel.toDouble / imageHeight + offsetY).toInt,
      zoom)
  }
  ...
  def visualizeGrid(
    ...
    val pixels = pixelLocations(x, y, zoom, imageWidth, imageHeight).map{
